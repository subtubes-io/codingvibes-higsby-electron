 ...
 * 
 * This script automates the complete build process for the ChatGPT extension:
...
console.log('🚀 Starting ChatGPT extension build and packaging process...\n');
...
try {
    ...
    const zipPath = path.join(__dirname, '../chatgpt-extension-ts.zip');
    if (fs.existsSync(zipPath)) {
        const stats = fs.statSync(zipPath);
        const sizeInKB = (stats.size / 1024).toFixed(2);
        
        console.log('\n🎉 ChatGPT extension packaging completed successfully!');
        console.log('📦 Package details:');
        console.log(`   📄 File: chatgpt-extension-ts.zip`);
        console.log(`   📏 Size: ${sizeInKB} KB`);
        console.log(`   📁 Location: ${zipPath}`);
        console.log('\n✨ Ready to upload and test in the graph editor!');
        console.log('\n🔑 Remember to set OPENAI_API_KEY environment variable for API functionality');
    } else {
